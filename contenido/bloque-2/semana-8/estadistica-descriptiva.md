...
print("¿Te has preguntado alguna vez cómo los analistas del Real Madrid procesan miles de estadísticas?")
...
print()
print("Pregunta de reflexión: ¿Cómo crees que estas herramientas cambiarán tu capacidad de descubrir insights?")
```
...
datos = {
    ...
    'Posicion': np.random.choice(['Delantero', 'Mediocampista', 'Defensor'], n_jugadores),
...
plt.title('¿Cómo varía el rendimiento ofensivo por especialización táctica?', size=16, pad=20)
...
plt.subplot(1, 2, 2)
plt.scatter(df_jugadores['Partidos'], df_jugadores['Goles'], alpha=0.6, color='orange', s=60)
...
top_eficientes = df_jugadores.nlargest(5, 'Eficiencia')[['Jugador', 'Posicion', 'Eficiencia', 'Goles', 'Partidos']]
...
sns.violinplot(data=df_jugadores, x='Posicion', y='Goles', ax=axes[0,0], palette='Set2')
axes[0,0].set_title('Distribución de Goles por Posición')
axes[0,0].tick_params(axis='x', rotation=45)

# Gráfico 2: Relación Edad-Rendimiento
sns.scatterplot(data=df_jugadores, x='Edad', y='Goles', hue='Posicion', ax=axes[0,1], s=80)
axes[0,1].set_title('Rendimiento vs Experiencia')

# Gráfico 3: Eficiencia por posición
sns.boxplot(data=df_jugadores, x='Posicion', y='Eficiencia', ax=axes[1,0], palette='Set3')
...
for candidato in [candidato_x, candidato_y]:
    ...
    if eficiencia > eficiencia_promedio:
        print(f"  ✓ Eficiencia superior al promedio del plantel ({eficiencia_promedio:.3f})")
    else:
        print(f"  ⚠️ Eficiencia inferior al promedio del plantel ({eficiencia_promedio:.3f})")
