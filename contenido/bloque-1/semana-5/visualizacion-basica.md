...
plt.scatter(goles_favor, puntos_obtenidos, s=120, alpha=0.7, color='steelblue', edgecolors='black')
...
print("Reflexión: ¿Qué otro cambio simple mejoraría claridad (ordenar, cambiar color, agrupar)?")
```
...
for i, e in enumerate(equipos):
    axes[1].annotate(e, (posesion[i], puntos[i]), xytext=(4,4), textcoords='offset points', fontsize=8)
