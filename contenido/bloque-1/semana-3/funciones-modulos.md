...
efectividad_ter_stegen = calcular_efectividad_portero(ter_stegen_tiros, ter_stegen_goles)
...
def analizar_rendimiento_jugador(nombre, goles, asistencias, partidos, posicion):
    ...
    if partidos == 0:
        return f"{nombre}: Datos insuficientes - No ha disputado partidos esta temporada"
...
# Delantero estrella
haaland_reporte = analizar_rendimiento_jugador(
    "Erling Haaland", 36, 8, 35, "delantero"
)
...
rudiger_reporte = analizar_rendimiento_jugador(
    "Antonio Rüdiger", 2, 1, 31, "defensa"
)
...
for simulacion in range(1, 6):
    ...
    print(f"Simulación {simulacion}: Real Madrid {goles_madrid} - {goles_barcelona} Barcelona ({resultado})")
...
print(f"Estilo completo: distancia = m1.sqrt((x2-x1)**2 + (y2-y1)**2) = {distancia1:.1f}m")

# Estilo 2: import específico
from math import sqrt as raiz
distancia2 = raiz((x2-x1)**2 + (y2-y1)**2)
print(f"Estilo específico: distancia = raiz((x2-x1)**2 + (y2-y1)**2) = {distancia2:.1f}m")
...
print(f"FC Barcelona: {barcelona_v}V-{barcelona_e}E-{barcelona_d}D = {barcelona_puntos} puntos")
...
print(f"Atlético Madrid: {atletico_v}V-{atletico_e}E-{atletico_d}D = {atletico_puntos} puntos")
print()
...
print(f"El Clásico: {comparar_equipos(madrid_puntos, barcelona_puntos, equipos_clasico)}")
print(f"Derbi madrileño: {comparar_equipos(madrid_puntos, atletico_puntos, equipos_derbi)}")
print(f"Rivalidad catalana: {comparar_equipos(barcelona_puntos, atletico_puntos, ['FC Barcelona', 'Atlético Madrid'])}")
...
def evaluar_jugador_elite(nombre, posicion, estadisticas):
    ...
    elif any(pos in posicion_lower for pos in ['mediocampo', 'mediocentro', '8', '10', 'volante']):
        # MEDIOCAMPISTAS: Equilibrio entre creatividad y eficiencia
        metric_principal = participaciones_por_partido * 0.7 + (precision_pases / 100) * 0.3
...
def analizar_plantilla_completa(nombre_equipo, jugadores_datos, resultados_recientes):
    ...
    for jugador_data in jugadores_datos:
        ...
        if 'error' not in evaluacion:
            ...
            total_asistencias_plantilla += jugador_data['estadisticas'].get('asistencias', 0)
    
    # ANÁLISIS DE FORMA RECIENTE (momentum del equipo)
    puntos_forma = sum(3 if resultado == 'V' else 1 if resultado == 'E' else 0 
    ...
    return {
        ...
        "metricas_globales": {
            ...
            "productividad_ofensiva": total_goles_plantilla + total_asistencias_plantilla
        },
        "analisis_forma": {
...
real_madrid_plantilla = [
    {
        ...
        "posicion": "Extremo",
        "estadisticas": {
            "goles": 24, "asistencias": 11, "partidos": 39, 
            "pases_exitosos": 892, "total_pases": 1180
        }
    },
    {
        ...
        "estadisticas": {
            "goles": 23, "asistencias": 13, "partidos": 42,
            "pases_exitosos": 1456, "total_pases": 1680
        }
    },
    {
        "nombre": "Luka Modrić",
        "posicion": "Mediocampista", 
        "estadisticas": {
            "goles": 3, "asistencias": 8, "partidos": 46,
            "pases_exitosos": 1998, "total_pases": 2234
        }
    },
    {
        ...
        "posicion": "Defensa",
        "estadisticas": {
            "goles": 2, "asistencias": 2, "partidos": 48,
            "pases_exitosos": 2156, "total_pases": 2398
        }
    }
]
...
barcelona_plantilla = [
    {
        ...
        "posicion": "Delantero",
        "estadisticas": {
            "goles": 26, "asistencias": 8, "partidos": 49,
            "pases_exitosos": 756, "total_pases": 943
        }
    },
    {
        "nombre": "Pedri",
        "posicion": "Mediocampista",
        "estadisticas": {
            "goles": 7, "asistencias": 8, "partidos": 37,
            "pases_exitosos": 1834, "total_pases": 2089
        }
    },
    {
        "nombre": "Gavi", 
        "posicion": "Mediocampista",
        "estadisticas": {
            "goles": 2, "asistencias": 13, "partidos": 51,
            "pases_exitosos": 2145, "total_pases": 2456
        }
    },
    {
        ...
        "posicion": "Defensa", 
        "estadisticas": {
            "goles": 6, "asistencias": 1, "partidos": 37,
            "pases_exitosos": 1567, "total_pases": 1789
        }
    }
]
...
madrid_analisis = analizar_plantilla_completa("Real Madrid", real_madrid_plantilla, madrid_forma_reciente)

print("ANALIZANDO FC BARCELONA...")
barcelona_analisis = analizar_plantilla_completa("FC Barcelona", barcelona_plantilla, barcelona_forma_reciente)
